...
import { createAchievementCard, renderAchievementCard } from "./share-card";
...
export default function Home() {
  ...
  const resultCardCanvas = useRef<HTMLCanvasElement>(null);
  ...
  useEffect(() => {
    if (view !== "result" || !resultCardCanvas.current) {
      setResultCardReady(false);
      return;
    }
    let active = true;
    setResultCardReady(false);
    void renderAchievementCard({ ...selected, issuedAt }, resultCardCanvas.current)
      .then(() => { if (active) setResultCardReady(true); })
      .catch(() => {
        if (active) setNotice("결과 카드를 표시하지 못했습니다. 잠시 후 다시 시도해 주세요.");
      });
    return () => { active = false; };
  ...
  const handleShare = async () => {
    setCardBusy("share");
    try {
      const card = await createAchievementCard({ ...selected, issuedAt }, resultCardCanvas.current);
    ...
    <main className={`app-shell view-${view} ${view === "result" && selected.id === "nothing" ? "view-result--mythic" : ""} ${secretQueue.length ? "is-secret-reveal" : ""}`}>
      ...
      {view === "home" && (
        <section className="home-screen screen-enter" aria-labelledby="home-title">
          ...
          <div className="home-actions">
            <button className="primary-button" onClick={() => setView("pick")}><span>네, 무려 뭔가 했습니다.</span><b>→</b></button>
      ...
      {view === "result" && (
        <section className="result-screen result-screen--card screen-enter" aria-live="polite" aria-label={`${selected.rarity} 등급 업적 ${selected.title} 달성 결과`}>
          <Confetti />
          <canvas className="result-card-canvas" ref={resultCardCanvas} width="1080" height="1350" aria-hidden="true" />
          <div className="result-actions">
            ...
            <button className="secondary-button secondary-button--more" onClick={() => { setChainStartedAt(Date.now()); setView("pick"); }}>하나 더 달성하기</button>
          </div>
      ...
      <nav className="bottom-nav" aria-label="주요 메뉴">
        <button className={view !== "collection" ? "active" : ""} onClick={() => setView("home")}><i>✦</i><span>오늘</span></button>
        <button className={view === "collection" ? "active" : ""} onClick={() => setView("collection")}><i>▦</i><span>업적 도감</span>{unlocked.length > 0 && <b>{unlocked.length}</b>}</button>
      </nav>
    </main>
  );
}
